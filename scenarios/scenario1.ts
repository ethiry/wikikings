...
import { BrowseData, ScenarioBase } from "./scenarioBase.ts";
...
export class Scenario1 extends ScenarioBase {
  ...
  protected override startData(): BrowseData[] {
    return [new BrowseData(this.startId)];
  }
  ...
  protected override mustStop(input: ItemId | WikiHuman): boolean {
    return (this.getIdFromInput(input) === this.endId);
  }

  protected override async continuationList(wiki: WikiHuman): Promise<BrowseData[]> {
    ...
    if (predecessors.length) {
      for (const pred of predecessors ?? []) {
        if (pred.replaces) {
          result.push(new BrowseData(pred.replaces));
        }
      }
    } else {
      // sibling king
      for (const id of wiki?.siblingsId ?? []) {
        ...
        if (sib instanceof WikiHuman && sib.isKing) {
          result.push(new BrowseData(sib));
        }
      }
    }
    ...
    // father
    if (wiki.fatherId) {
      result.push(new BrowseData(wiki.fatherId, true));
    }
